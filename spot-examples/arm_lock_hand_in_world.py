...
def lock_hand_in_world_and_move(config):
    """A simple example of using the Boston Dynamics API to command Spot's arm.
    Spot will set an arm pose, then it will move forward by 2 meters.
    The expected result will be spot is moving with the hand locked in the world reference position.
    It is the same behavior as with the remote controller by activating the LOCK_HAND_IN_WORLD option.
    """
...
def main(argv):
    ...
    try:
        lock_hand_in_world_and_move(options)
        return True
